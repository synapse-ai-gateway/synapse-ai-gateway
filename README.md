...
- **One command, real machines.** `docker compose up` brings the whole stack — postgres + backend + admin console — in five minutes. Backend is a 195 MB image and uses under 100 MB idle RSS. Runs on a laptop for development and on a single VM for a small team. No Redis, no Celery, no message broker required to get started.
